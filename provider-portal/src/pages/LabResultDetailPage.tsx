...
const StatusBadge: React.FC<{ interpretation: string }> = ({
  interpretation,
}) => {
  const config = {
    N: {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${config.color} ${config.bg} border ${config.border}`}
    >
      {config.label}
    </span>
  );
...
const TrendIndicator: React.FC<{ trend: string }> = ({ trend }) => {
  const config = {
    ...
    RECENT_UP: {
      symbol: "──↑",
      color: "text-blue-500",
      label: "Recently increased",
    },
    RECENT_DOWN: {
      symbol: "──↓",
      color: "text-orange-500",
      label: "Recently decreased",
    },
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  ...
                  Encounter:{" "}
                  <span className="font-medium">{result.encounterId}</span> |
          ...
          <div className="px-6 py-5 border-t border-gray-200">
            <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">
              📈 Historical Comparison (Last{" "}
              {result.historicalResults.length + 1} Results)
            </h2>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
            ...
            <div className="mt-4 text-xs text-gray-500">
              <p>
                💡 <strong>Tip:</strong> Values in green represent the most
                recent result. Compare trends over time to monitor patient
                progress.
              </p>
            </div>
          </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-blue-700">
                <strong>Note:</strong> This is a demonstration of the lab
                results display system based on international EMR standards
                (Epic, Cerner, HL7 FHIR). The actual implementation will
                integrate with the lab service API and support multiple test
                types (CMP, Lipid Panel, Thyroid Panel, etc.).
              </p>
            </div>
          </div>
        </div>
