...
They sit alongside `DEVELOPMENT_LAW.md` and **MUST** be followed for every PR that touches behavior, data, or UI.
...
- Rules:
  ...
  - Services MUST use `PrismaService` for all DB access.
...
- Builder functions (e.g. `buildPhysicalExamObservations`):
...
- Any deviation from this law MUST be explicitly documented in the PR description and approved by the project owner.
